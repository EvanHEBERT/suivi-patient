...
app.post("/api/analyze-conversation", upload.single("audio"), async (req, res) => {
  try {
    ...
    if (err.message && err.message.includes("[400 Bad Request]")) {
      errorMessage = "Erreur de requête Gemini (400). Vérifiez le prompt ou les données envoyées.";
      ...
      errorMessage = "Modèle Gemini non trouvé (404). Assurez-vous que 'Vertex AI API' est activé sur votre projet Google Cloud.";
    } 
